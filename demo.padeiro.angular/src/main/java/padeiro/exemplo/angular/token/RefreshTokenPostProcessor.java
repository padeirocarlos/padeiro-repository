...
public class RefreshTokenPostProcessor implements ResponseBodyAdvice<OAuth2AccessToken> {
	...
	@Inject
	private
	PadeiroApiProperty padeiroApiProperty;
	...
	private void addRefreshTokenNoCookie(String refreshToken, HttpServletRequest req, HttpServletResponse resp) {
		...
		refreshTokenCookie.setHttpOnly(getPadeiroApiProperty().getHttpOnly().isEnableHttpOnly());
		refreshTokenCookie.setSecure(getPadeiroApiProperty().Security().isEnableHttps()); // TODO: Mudar para true em producao
		refreshTokenCookie.setPath(req.getContextPath() + ETokenConfig._OAUTH_TOKEN.getValue());
